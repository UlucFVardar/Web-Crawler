...
def googleNewsCrawler():
    ...
    sourceSoup=BeautifulSoup(plain_text)
    #---
    ...
    for innerSoup1 in sourceSoup.find_all('div', {'class': 'WyeMbd'}):
        innerSoup1Source = BeautifulSoup(str(innerSoup1))
        for innerSoup2 in innerSoup1Source.find_all('c-wiz', {'data-label': 'En Çok Okunan Haberler'}):
            innerSoup2Source=BeautifulSoup(str(innerSoup2))
            for innerSoup3 in innerSoup2Source.find_all('div',{'class':'deQdld'}):
                innerSoup3Source = BeautifulSoup(str(innerSoup3))
                for innerSoup4 in innerSoup3Source.find_all('c-wiz',{'class':'M1Uqc kWyHVd'}):
                    innerSoup4Source = BeautifulSoup(str(innerSoup4))
    ...
    for innerTag1 in sourceSoup.find_all('div', {'class':'QWD7rd JHzJp'}):
        innerTag1Source=BeautifulSoup(str(innerTag1))
        for innerTag2 in innerTag1Source.find_all('c-wiz', {'jsdata': 'deferred-i7'}):
            innerTag2Source=BeautifulSoup(str(innerTag2))
